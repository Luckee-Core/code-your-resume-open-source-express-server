...
export const buildCandidateIdentityPromptSection = (
  ...
  const headline = input.profile?.headline?.trim() ?? '';

  const sublineParts = [headline, location].filter(Boolean);
  const sublineHint = sublineParts.length
    ? `Optional one-line subline under the name (pick at most one): ${sublineParts.join(' · ')}`
    : 'Optional subline: a 3–6 word role label aligned to the job title.';
