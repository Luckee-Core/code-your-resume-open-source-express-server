-- Cover letter prompt v4: projects replace credibility_bio; voice_style from voice_style table.
-- Run against CRM Supabase after supabase-cover-letter-point-of-emphasis-prompt-v2.sql.
...
### Projects (factual work history — do not invent beyond this)
...
- **Required letter structure (all required):**
  ...
  2) **Line 2+ (new paragraph)** — MUST start with `My name is [Full Name],` using the candidate's full name from projects/notes (name only — **no** "Founder", "Co-founder", or similar after the name). Then state clearly that you are **applying for** (or **interested in**) the **{{jobTitle}}** role at the company — you want to join their team, not sell them a service
...
- **Read projects first** for factual work history and name. Paragraph 1 is only `Hola hola,`. Paragraph 2 MUST begin `My name is [Full Name],` (name from projects — no founder/co-founder title) and make clear you are **applying for the role** / want to **join the team**.
...
- **Truthfulness:** Do not invent employers, degrees, certifications, or metrics unsupported by the projects/skills context.
